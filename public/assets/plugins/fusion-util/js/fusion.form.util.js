 ...
 */
const fetchReq = ({uri = '', method = 'get', data = null, dataType = 'json', timeout = 8000, beforeSend, onComplete, onSuccess, onError}) => {
...
class FBUtil extends FBBase {
    ...
    // TODO: Refactor isFormElement() - Should not have a parameter
    /**
    ...
    mouseIsOver() {
        ...
        return $fs(target).selectorMatches(':hover')
    }
    ...
    toggleSubmitButtonState(processIsDone = false) {
        ...
        if (target.length) {
            if (this.isFormElement(target[0])) {
                const submitButton = $fs('button[type="submit"]', _target).length ?
                    $fs('button[type="submit"]', _target) :
                    ($fs(`button[form="${target[0].id}"]`).length ? $fs(`button[form="${target[0].id}"]`) : $fs(`a[form="${target[0].id}"]`));
     ...
     * @param complete {function|null}
     * @param completed {function|null}
     * @return {void|FBUtil}
     */
    onBSModalLoad(options, complete = null, completed = null) {
        const _target = this, target = _target.target;
        if (target.length) {
            _target.upon('show.bs.modal', isFunction(complete) ? complete : (isFunction(options) && options));
            _target.upon('shown.bs.modal', isFunction(completed) ? completed : (isFunction(options) && options));
            if (target.length > 1)
    ...
    onBSModalClose(callback) {
        ...
        if (target.length) {
            _target.upon('hide.bs.modal', function (e) {
                isFunction(callback) && callback(e, _target);
            });
        }
    }
                ...
                fetchReq({
                    ...
                    onSuccess: (data) => {
                        ...
                        if (selector) {
                            ...
                            if (!hasError) {
                                newData = newData.replaceAll(/(\\")+/gi, '"').replaceAll(/(\\r)?(\\n)?(\\t)?/gi, '').replace(/^"/, '').replace(/"$/, '');
                                _element.html.insert(newData);
                                typeof callback === 'function' && callback();
                                resolve({responseText: data.responseText, response: newData, status: data.status});
                            }
                            reject(error)
                        } else {
                            newData = data.responseText.replaceAll(/(\\")+/gi, '"').replaceAll(/(\\r)?(\\n)?(\\t)?/gi, '').replace(/^"/, '').replace(/"$/, '');
                            _element.html.insert(newData);
                            typeof callback === 'function' && callback();
                            resolve({responseText: data.responseText, response: newData, status: data.status});
                        }
     ...
     * @return {Promise<unknown>}
     */
    handleFormSubmit({uri = '', method = 'get', data = null, dataType = 'json', beforeSend = null, timeout = 3000} = {}) {
            ...
            target.forEach(function (element) {
                if (_target.isFormElement(element)) {
                    ...
                    !form.hasErrors ? fetchReq({
                        uri: _element.action ?? uri,
                        method: _element.formMethod ?? method,
                        data: formData,
                        dataType: dataType,
                        timeout: timeout,
                        beforeSend: () => {
                            ...
                            setTimeout(() => {
                                if ((status > 199 && status < 300 || status === 308)) {
                                    ...
                                    if (dataType === 'json') {
                                        ...
                                        else if (status === 422 || status === 511) {
                                            console.log(_messageTag, response.message)
                                            !!response.errors && _element.validator.renderValidationErrors(response.errors, response.message);
                                            _messageTag.length && _messageTag.renderMessage(fa_exc_c, alert_d, response.message, null, element, true);
                                            _messageTag === _globalMessageTag && _messageTag.slideInDown(800);
                                            resolve({JSON: response, text: responseText, form: _element, messageTag: _messageTag});
                                        } else {
...
class FBBSModal extends FBUtil {
     ...
     * @param onCompleted
     */
    loadModal({route, modal, options, beforeOpen, onComplete, onCompleted}) {
        const _target = this;
        
        if (_target.length)
            _target.length && _target.loadPageData({
                uri: route, selector: modal, beforeSend: beforeOpen, callback: (e) => {
                    $fs(modal).onBSModalLoad(options, function (e) {
                        isFunction(onComplete) && onComplete({target: e, response: $fs(modal), status: 'success'});
                    }, function (e) {
                        isFunction(onCompleted) && onCompleted({target: e, response: $fs(modal), status: 'success'});
                    });
                }
     ...
     * @param onCompleted
     */
    onClickOpen({beforeOpen = null, options = null, onComplete = null, onCompleted = null} = {}) {
        const _target = this;
        ...
        if (_target.length)
            _target.upon('click', function (e) {
                ...
                _targetWrapper.length && _targetWrapper.modal.loadModal({route: route, modal: targetModal, beforeOpen: beforeOpen, onComplete: onComplete, onCompleted: onCompleted});
            });
    }
}
                        ...
                        if (originalLabel.length && originalField.length) {
                            ...
                            _inputElement.upon({
                                focus: (e) => {
                                    ...
                                    if (this.isPasswordField(config.passwordId)) {
                                        if (config.showPassword && _toggler.length) {
                                            if (_toggler.mouseIsOver() && _inputElement[0].value.length) {
                                ...
                                keyup: (e) => {
                                    this.#_resetFBObject(_inputElement);
                                    const dimensions = this.#_getFieldDimensions();
                                    const capslockIsOn = e.constructor.name === 'KeyboardEvent' ? e.getModifierState('CapsLock') : false;
                                    const _toggler = $fs(toggler, $fs(target));
...
Array.prototype.contains = function (value) {
    return !!this.filter(val => value === val).length;
}
